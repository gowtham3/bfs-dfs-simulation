...
class Edge:
    ...
    def draw_edge(self,screen):
        if not Edge.directed:
            ...
            pygame.draw.line(screen,self.color,(self.start_x,self.start_y),(self.end_x,self.end_y))
            draw_arrow(self.start_x,self.start_y,self.end_x,self.end_y,screen,self.color)
...
def draw_arrow(x1,y1,x2,y2,screen,color):
...
def is_near(x,y):
    for node in Node.all_nodes:
        if (x > (node.x - 2*Node.radius)) and (x < (node.x + 2*Node.radius)) and (y > (node.y - 2*Node.radius)) and (y < (node.y + 2*Node.radius)):
            return True
    return False
...
def is_in(x,y):
    for node in Node.all_nodes:
        if (x > (node.x - Node.radius)) and (x < (node.x + Node.radius)) and (y > (node.y - Node.radius)) and (y < (node.y + Node.radius)):
            return node
    return False
